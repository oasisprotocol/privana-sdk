...
async function resolveDeliveredAmount({
  ...
  try {
    const receipt = await getTransactionReceipt(wagmiConfig, {
      hash: record.on_chain_tx_hash as `0x${string}`,
      chainId: record.chain_id,
    })
    ...
    for (const log of receipt.logs) {
      ...
      try {
        ...
        if (to !== record.wallet_address.toLowerCase()) continue
  ...
  // For ERC-20 MoonPay deliveries, the chain receipt is authoritative. The
  // MoonPay sandbox display amount can use a different decimal convention and
  // must not be sent to `/deposits/check` as the Privana claim amount.
  const errorDetail =
    receiptError instanceof Error
      ? receiptError.message
      : receiptError === undefined
        ? `no ${token.symbol} Transfer to ${record.wallet_address} found`
        : String(receiptError)
  throw new Error(
    `Unable to derive delivered ${token.symbol} amount from ${record.on_chain_tx_hash}: ${errorDetail}`
  )
}
